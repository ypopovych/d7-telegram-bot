...
import { Context, MatchedContext, Storage } from "../types"
...
export function isBotCommand(ctx: MatchedContext<Context, 'message'>): boolean {
    ...
    if (ctx.simpleCommands.includes(match[1] ?? '')) return true
...
export async function ensureChatAdmin(ctx: MatchedContext<Context, 'message'>, user: User): Promise<boolean> {
    if (!await ensureGroupChat(ctx)) return false
    if (!await isChatAdmin(ctx.telegram, ctx.storage, String(ctx.chat.id), user.id)) {
        await ctx.reply(
